...
void prewitt_stencil(const int rows, const int cols, pixel * const in, pixel * const out) {
	const int radius = 1;
...
void apply_stencil(const int radius, const double stddev, const int rows, const int cols, pixel * const in, pixel * const out) {
	...
	parallel_for(blocked_range<int>(0, rows), Stencil_class(radius, rows, cols, dim, in, out, kernel));
}
...
int main( int argc, char* argv[] ) {
	...
	// Do the stencil
	apply_stencil(3, 32.0, rows, cols, imagePixels, outPixels2);
	prewitt_stencil(rows, cols, outPixels2, outPixels);
